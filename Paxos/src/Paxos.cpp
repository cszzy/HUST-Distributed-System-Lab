...
const int num_proposer = 10;
const int num_acceptor = 11;

paxos::Proposer p[num_proposer];
paxos::Acceptor a[num_acceptor];
mdk::Mutex l[num_acceptor];
int finishedCount = 0;
int finalValue = -1;
...
int main(int argc, char* argv[])
{
	...
	g_log.Info("Info", "%d个Proposer, %d个Acceptor准备进行Paxos\n"
		"每个Proposer独立线程，Acceptor不需要线程\n"
		"Proposer编号从0-10,编号为i的Proposer初始提议编号和提议值是（i+1, i+1）\n"
		"Proposer每次重新提议会将提议编号增加%d\n"
		"Proposer被批准后结束线程,其它线程继续投票最终，全部批准相同的值，达成一致。\n", num_proposer, num_acceptor, paxos::num_proposal_inc);
	g_start = mdk::MillTime();
	g_log.Info("Info", "Paxos开始\n" );
	paxos::PROPOSAL value;

	for ( i = 0; i < num_proposer; i++ ) 
	{
		p[i].SetPlayerCount(num_proposer, num_acceptor);
		value.serialNum = value.value = i + 1;
		p[i].StartPropose(value);
	}

	mdk::Thread t[num_proposer];
	for ( i = 0; i < num_proposer; i++ ) t[i].Run(Proposer, (void*)i);
	//for ( i = 0; i < num_proposer; i++ ) t[i].WaitStop();
	while(true){
		if(isFinished) break;
		mdk::m_sleep(500);
	}
