...
class sim_cache:
    ...
    def read(self,target_file):
        ...
        ####### Begin #######
        #检查缓存中是否已缓存文件
        is_incache = self._search_cache(target_file)
        #向server确认version是否一致
        server_version = self._target_server.get_version(target_file)
        #选择从cache/server中读取文件
        if is_incache == True:
            cache_res = self._read_cache(target_file)
            if cache_res[2] != server_version:
                # read from server
                server_res = self._target_server.read(target_file)
                self._write_cache(target_file, server_res[1], server_version)
                return server_res[1]
            return cache_res[1]
        else:
            server_res = self._target_server.read(target_file)
